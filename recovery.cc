...
private:
  ...
  TGraph *g_onepar_resi[MAXCH], *g_twopar_resi[MAXCH];
  ...
  string mem_twopar_func = "[2]*x/(1+[0]*x)+(1-[2])*x/(1+[1]*x)";
  string mem_anadate;
...
void Analizer::setValue(){
  ...
  TTree *integ_tree = (TTree*)integ_file -> Get("integral");
  ...
  mem_cycle = integ_tree -> GetEntries();
  for(int cy=0; cy<mem_cycle; cy++){
    integ_tree -> GetEntry(cy);
    integ_err_tree -> GetEntry(cy);
    for(int ch=0; ch<mem_readch; ch++){
      mem_integral_mean[ch][cy] = integ_tree -> GetLeaf(Form("ch%d",ch)) -> GetValue(0);
      mem_integral_err[ch][cy] = integ_err_tree -> GetLeaf(Form("ch%d",ch)) -> GetValue(0);
    }
  }
  dark_file -> Close();
  integ_file -> Close();
...
void Analizer::anaAdc2Photon(){
  for(int ch=1; ch<mem_readch; ch++){
    ...
    f_linear[ch] = new TF1(Form("f_linear_ch%d",ch),"[0]*x+[1]", 0, 1.0e5);
    g_adc[ch] -> Fit(Form("f_linear_ch%d",ch),"Q0","",0,mem_integral_mean[0][mem_linearfit_num]);
...
void Analizer::anaRecovery(){
  ...
  for(int ch=1; ch<mem_readch; ch++){
    ...
    g_photon[ch] -> Fit(Form("f_onepar_ch%d",ch), "Q0", "", 500, mem_fit_max);
    ...
    g_photon[ch] -> Fit(Form("f_twopar_ch%d",ch), "Q0", "", 500, mem_fit_max);
    for(int cy=0; cy<mem_cycle; cy++){
    ...
    g_onepar_resi[ch] = new TGraph(mem_cycle, mem_nref_mean[ch], mem_onepar_residual[ch]);
    g_twopar_resi[ch] = new TGraph(mem_cycle, mem_nref_mean[ch], mem_twopar_residual[ch]);
    ...
    mem_twopar_beta[ch] = 1 - f_twopar[ch]->GetParameter(2);
...
void Analizer::makeCanvas(){
  ...
  for(int ch=1; ch<mem_readch; ch++){
    canvas[ch] = new TCanvas("test","test",50,50,1200,900);
    canvas[ch] -> Divide(4,3,0.001,0.001);
    canvas[ch] -> cd(1);
    g_adc[ch] -> Draw("ap");
    ...
    canvas[ch] -> cd(6) -> DrawFrame(0,0.9,1.1*mem_nref_mean[ch][mem_cycle-1],1.1,"one parameter residual;Nref (#mu s^{-1});Data/Fit");
    g_onepar_resi[ch] -> Draw("p");
    f_residual_base -> Draw("same");
    canvas[ch] -> cd(7) -> DrawFrame(0,0.9,mem_fit_max,1.1,"one parameter residual (fit region);Nref (#mu s^{-1});Data/Fit");
    g_onepar_resi[ch] -> Draw("p");
    f_residual_base -> Draw("same");
    canvas[ch] -> cd(8);
    ...
    canvas[ch] -> cd(10) -> DrawFrame(0,0.9,1.1*mem_nref_mean[ch][mem_cycle-1],1.1,"two parameter residual;Nref (#mu s^{-1});Data/Fit");
    g_twopar_resi[ch] -> Draw("p");
    f_residual_base -> Draw("same");
    canvas[ch] -> cd(11) -> DrawFrame(0,0.9,mem_fit_max,1.1,"two parameter residual (fit region);Nref (#mu s^{-1});Data/Fit");
    g_twopar_resi[ch] -> Draw("p");
    f_residual_base -> Draw("same");
    canvas[ch] -> cd(12);
